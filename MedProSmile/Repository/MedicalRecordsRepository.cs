...
namespace MedProSmile.Repository
{
    public class MedicalRecordsRepository : IMedicalRecordsRepository
    {
        ...
        public async Task<PagedResult<dynamic>> GetAllPagedAsync(int? doctorId, int hospitalId, int pageNumber, int pageSize)
        {
            try
            {
        ...
        public async Task<dynamic> GetByIdAsync(int id, int hospitalId)
        {
            try
            {
                ...
                var parameters = new { RecordId = id, HospitalId = hospitalId };
                using var connection = _context.CreateConnection();
