...
namespace MedProSmile.Repository
{
    public class EmployeeRepository : IEmployeeRepository
    {
        ...
        public EmployeeRepository(DapperContext context, IExceptionLogger exceptionLogger)
        {
            _context = context;
        ...
        public async Task<int> CreateAsync(Employee emp)
        {
            try
            {
                var query = "usp_CreateEmployee";
                var parameters = new { emp.Name, emp.Designation, emp.Salary };
                using var connection = _context.CreateConnection();
        ...
        public async Task<int> UpdateAsync(Employee emp)
        {
            try
            {
                var query = "usp_UpdateEmployee";
                var parameters = new { emp.EmployeeId, emp.Name, emp.Designation, emp.Salary };
                using var connection = _context.CreateConnection();
                return await connection.ExecuteAsync(query, parameters, commandType: CommandType.StoredProcedure);
            }
            catch (Exception ex)
